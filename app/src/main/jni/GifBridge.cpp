...
jobject getGifBitmap(JNIEnv* env, jclass clazz, jstring fileName) {
  ...
  GifDecoder *gifResult = loadGif(fileNameChars);
  env->ReleaseStringUTFChars(fileName, fileNameChars);
  LOGD("width = %i, height = %i", gifResult->width, gifResult->height);
   // Creaing Bitmap Config Class
      ...
      // Creating a Bitmap Class
       int imgWidth = gifResult->width;
       int imgHeight = gifResult->height;
      ...
      if(gifResult->width > 0 && gifResult->height > 0) {
            int pixelsCount = gifResult->width * gifResult->height;
            //LOGD("before get result");
            // gifResult->getPixels();
            // LOGD("pixels count = %i", pixelsCount);
            memcpy(bitmapPixels, gifResult->getPixels(), pixelsCount*4);
      }
  return jBmpObj;
}
